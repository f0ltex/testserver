...
local reviveWaitPeriod = 5 -- How many seconds to wait before allowing player to revive themselves
...
AddEventHandler('onClientMapStart', function()
	Citizen.Trace("SASERP: Disabling autospawn...")
...
AddEventHandler('RPD:allowRespawn', function(from)
	TriggerEvent('chatMessage', "SASERP", {200,0,0}, "Respawned")
	allowRespawn = true
end)
...
AddEventHandler('RPD:allowRevive', function(from)
	...
	if(GetPlayerServerId(PlayerId()) == from and diedTime ~= nil)then
		...
		if(GetGameTimer() < waitPeriod)then
			...
			TriggerEvent('chatMessage', "SASERP", {200,0,0}, "You must wait before reviving yourself, you have ^5"..message.."^0 remaining.")
			return		
		end
	end

	-- Revive the player.
	TriggerEvent('chatMessage', "SASERP", {200,0,0}, "Revived")
	allowRevive = true
end)
...
AddEventHandler('RPD:toggleDeath', function(from)
	...
	if (RPDeathEnabled) then
		TriggerEvent('chatMessage', "SASERP", {200,0,0}, "RPDeath enabled.")
	else
		TriggerEvent('chatMessage', "SASERP", {200,0,0}, "RPDeath disabled.")
	end
end)
